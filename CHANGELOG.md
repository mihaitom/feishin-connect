...
- **Japanese furigana/romaji lyrics overlays now also work in the web/Docker build** - upstream's new furigana/romaji conversion only runs in the Electron main process (needs Node to run kuroshiro/kuromoji), so it silently did nothing outside the desktop app. The Connect backend now provides the same conversion via `pykakasi`, and the frontend falls back to it when not running in Electron. Word segmentation quality is slightly different from kuroshiro's (dictionary-based vs. statistical), but close enough for lyrics.
...
- **Merged upstream Feishin v1.14.0** — headline change is a large lyrics rewrite: a new karaoke-style word-by-word lyrics view, Japanese furigana/romaji overlays, and OpenSubsonic structured (word-level) lyrics support. Also adds a Subsonic jukebox integration and a "stopped" playback state. Synced-lyrics scroll/seek and the karaoke view now also follow a Connect device's playback position when one is active, matching the existing classic lyrics view.
