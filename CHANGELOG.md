...
### Fixed
- **Logging into a second Navidrome account failed with "User is not authorized for given operation" when Navidrome sits behind an SSO reverse proxy (e.g. Authentik ForwardAuth)** — the Connect backend's Navidrome proxy forwarded every request header verbatim, including SSO identity headers like `X-authentik-username`. If Navidrome trusts the proxy's source IP for that header (`ND_EXTAUTH_TRUSTEDSOURCES`), it silently authenticated every proxied request as whoever is browsing instead of the Subsonic credentials actually being sent — breaking login as any account other than your own SSO identity, even an admin one. These forward-auth headers are now stripped before requests reach Navidrome.
- **The Connect popover closed itself as soon as a device was disconnected/deselected**, or as soon as a connection attempt succeeded — picking a new device right after either one meant re-opening the popover first. It now only closes when you explicitly click "Connect"/"Add" or "Disconnect all".
- **The player-bar Stop button disconnected the Connect device entirely** instead of just stopping playback on it — every "Stop" click meant a full reconnect just to play the next track. It now pauses and resets to 0:00, same as a normal stop, without releasing the device.
- **Clicking "Connect" sometimes silently did nothing** — it only sent the actual cast request if local playback happened to be in the exact "playing" state at that instant; paused (or not-yet-started) queues were silently ignored while the UI still showed the device as connecting. Connecting now works regardless of local play/pause state.
- **Album art sent to Sonos/Chromecast/AirPlay/DLNA devices could be unreachable when the media server sits behind `SERVER_INTERNAL_URL`** (e.g. an SSO-protected Navidrome/Jellyfin) — cover art URLs were built from the browser-facing address, which cast devices on the LAN can't necessarily reach, instead of the internal one already used for the audio stream itself.
- **The play/pause button showed "Play" instead of "Pause" while streaming radio to a Connect device**, and clicking it controlled local radio playback instead of the connected device — the radio-specific play button (shown whenever radio is active) never checked Connect state at all, unlike the regular queue play button and the radio stop button next to it. Same underlying gap also affected the mobile fullscreen player's play/pause button for both radio and regular queue playback. Both now reflect and control the connected device's play state when Connect is active.
- **Pausing radio on a Connect device and pressing Play again did nothing** — resuming (and seeking) always reconnected the device to the backend's track-streaming endpoint, which has nothing to serve for radio (radio streams straight from its own URL, no track loaded). The device silently got no audio back. Resume/seek — and joining an additional device mid-stream — now correctly reconnect radio to its own URL.
- **Clicking "Connect" with nothing loaded in the queue (e.g. right after a page reload, before picking a track) failed silently every time** — only the takeover flow had a fallback for connecting with nothing to play yet; the plain "Connect" button just refused with no visible error and never actually claimed the device. It now claims the device the same way takeover already does, ready to play as soon as a track is picked.
- **The Connect backend session can still be finishing setup (its credential re-validating against the media server) right when "Connect" is clicked**, e.g. right after a page reload — connecting during that window now waits for setup to finish instead of racing it, and the cast button shows a spinner while it does instead of looking unresponsive.
