...
// When a Connect device is active, lyric sync follows its reported elapsed time
// and play state instead of the local player's — see connect.store.ts.
export const getEffectiveLyricsStatus = (): PlayerStatus => {
    const connect = useConnectPlayerStore.getState();
    if (connect.isActive) {
        return connect.isPlaying ? PlayerStatus.PLAYING : PlayerStatus.PAUSED;
    }
    return usePlayerStoreBase.getState().player.status;
};

export const getEffectiveLyricsTimestamp = (): number => {
    const connect = useConnectPlayerStore.getState();
    if (connect.isActive) {
        return connect.isPlaying
            ? connect.elapsed + (Date.now() - connect.syncTime) / 1000
            : connect.elapsed;
    }
    return useTimestampStoreBase.getState().timestamp;
};

// Fires on Connect activation/deactivation, play/pause, or a device-side
// elapsed correction (2s poll or remote seek) — the Connect-store analog of
// subscribePlayerStatus/subscribePlayerProgress for the local player.
export const subscribeConnectLyricsChanges = (onChange: () => void) =>
    useConnectPlayerStore.subscribe((state, prevState) => {
        if (
            state.isActive === prevState.isActive &&
            state.isPlaying === prevState.isPlaying &&
            state.elapsed === prevState.elapsed &&
            state.syncTime === prevState.syncTime
        ) {
            return;
        }
        onChange();
...
export const useSynchronizedLyricsBase = (settingsKey = 'default', offsetMs?: number) => {
    ...
    const { isActive: connectActive } = useConnectPlayerStore();
