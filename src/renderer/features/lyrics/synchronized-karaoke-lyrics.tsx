...
import {
    getEffectiveLyricsStatus,
    getEffectiveLyricsTimestamp,
    LYRICS_SCROLL_CONTAINER_ID,
    subscribeConnectLyricsChanges,
...
import { subscribePlayerStatus } from '/@/renderer/store';
...
export const SynchronizedKaraokeLyrics = ({
    ...
    const playbackAnchorRef = useRef({
        ...
        timeMs: getEffectiveLyricsTimestamp() * 1000,
    });
    ...
    const syncFromCurrentTimestamp = useCallback(() => {
        const timestamp = getEffectiveLyricsTimestamp();
    ...
    useEffect(() => {
        ...
        const unsubscribe = subscribePlayerStatus(({ status }) => {
            if (useConnectPlayerStore.getState().isActive) return;
    ...
    // Connect device active/inactive, play state, or a device-side elapsed
    // correction (2s poll or remote seek) — follow the same handoff as the
    // local-player subscription above, but keyed off the Connect store.
    useEffect(() => {
        const unsubscribe = subscribeConnectLyricsChanges(() => {
            const status = getEffectiveLyricsStatus();
            statusRef.current = status;
        ...
        return unsubscribe;
    }, [resumeAutoscroll, resumeEngineAutoscroll, startRaf, stopRaf, syncFromCurrentTimestamp]);
